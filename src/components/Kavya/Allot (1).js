...
const Allot = () => {
    ...
    const fetchData = async () => {
      try {
        const [driversResponse, vehiclesResponse, routesResponse] = await Promise.all([
          axios.get('https://location-qxki.onrender.com/driver'),
          axios.get('https://location-qxki.onrender.com/vehicle'),
          axios.get('https://location-qxki.onrender.com/route')
        ]);
  ...
  const handleUpdate = async (driverId, vehicleNo, routeId , vehicleName , routeName) => {
    ...
    try {
      await axios.put(`https://location-qxki.onrender.com/update-driver/${driverId}`, {
        vehicleNo,
        routeId,
        vehicleName,
        routeName
      ...
      // Update the drivers state to reflect the changes
  ...
  return (
    <div className="p-6 bg-gray-50 dark:bg-gray-800 rounded-lg shadow-md">
      <ToastContainer />
      <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-6">Allot Vehicles and Routes</h2>
                ...
                <td className="px-6 py-4">
                  <select
                    onChange={(e) => {
                      ...
                      const routeName = routes.find(r => r._id === routeId)?.name || '';
